...
class BaseExceptionCallbackImpl(ExceptionCallback):
  ...
  @classmethod
  def create(cls, context: dict[str, Any], meta: CreateExceptionCallbackMeta) -> 'BaseExceptionCallbackImpl':
    instance = cls()
...
class ExceptionCallbackImpl(BaseExceptionCallbackImpl):
  def __call__(self, exc_type: Type[BaseException], exc_value: BaseException, exc_traceback: TracebackType | None):
